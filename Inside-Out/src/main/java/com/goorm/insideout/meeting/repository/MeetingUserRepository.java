...
public interface MeetingUserRepository extends JpaRepository<MeetingUser, Long> {
	...
	@Query("select m from MeetingUser m where m.user.id = ?1 and m.approvalStatus = ?2 and m.meeting.progress = ?3")
	List<MeetingUser> findOngoingMeetingsByProgress(Long id, ApprovalStatus approvalStatus, Progress progress);
	...
	@Query("select m from MeetingUser m where m.user.id = ?1 and m.approvalStatus = ?2 and m.meeting.progress = ?3")
	List<MeetingUser> findEndedMeetings(Long id, ApprovalStatus approvalStatus, Progress progress);
